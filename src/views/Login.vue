      ...
      <v-card class="pa-0" height="550" width="400" style="overflow: hidden; background-image: linear-gradient(to top right, #f9a743, #f9db5f);">
        <v-card-text>
          ...
          <v-form ref="form" v-model="valid" lazy-validation>
            <v-text-field prepend-inner-icon="mdi-account" v-model="username" label="Username" outlined rounded hide-details class="mb-3 username" @focus="usernameFocusEvent" @input="usernameInputEvent" @blur="usernameBlurEvent" required />
            <v-text-field prepend-inner-icon="mdi-lock" v-model="password" :type="showPass ? 'text' : 'password'" :append-icon="showPass ? 'mdi-eye' : 'mdi-eye-off'" @click:append="togglePass" label="Password" outlined rounded hide-details class="password" @focus="passwordFocusEvent" @blur="passwordBlurEvent" required />  
          </v-form>
        </v-card-text>  
        <v-card-actions>
          <v-spacer />
          <v-btn :disabled="!valid" width="150" rounded large color="primary" dark class="mr-4" @click.prevent="login"> Login </v-btn>  
          <v-spacer />
        </v-card-actions>
...
<script>
  export default {
    ...
    methods: {
      ...
      passwordFocusEvent() {
        console.log('passwordFocusEvent called')  
        document.querySelectorAll('.hand').forEach(hand => {
